...
| **Three Reading Views** — original text, side-by-side dual-language, sentence-by-sentence parallel | **三种阅读视图** — 原文 / 双栏对照 / 逐句对照，随时切换 |
...
2. Download `ReadCoachAI.html` and open it in any modern browser — no server, no install
...
2. 下载 `ReadCoachAI.html`，用任意现代浏览器打开，无需服务器、无需安装
...
```
ReadCoachAI/
├── ReadCoachAI.html      # Standalone single-file app (primary)
│                      # 单文件独立版本（主要开发文件）
...
The `src/` version requires a local server (e.g. VS Code Live Server or `python -m http.server`). `ReadCoachAI.html` opens directly by double-click.

`src/` 拆分版需要本地服务器运行（如 VS Code Live Server 或 `python -m http.server`）。`ReadCoachAI.html` 双击即可直接打开。
